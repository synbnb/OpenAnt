...
func TestIndexKeepsLegacyKeyFieldWithoutV2Config(t *testing.T) {
	home := t.TempDir()
	writeTestLLMConfig(t, home, `{}`)
	body := renderIndexForTest(t, filepath.Join(home, "webui"))
	for _, want := range []string{"openant-default", "anthropic", "LLM API Key (legacy Anthropic mode)"} {
		if !strings.Contains(body, want) {
			t.Errorf("legacy index is missing %q", want)
		}
	}
