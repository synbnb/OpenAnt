...
// SourceCodeBaseEnv is an optional override for the project-local source
// corpus. It is intentionally an environment variable rather than a user
// specific absolute path so packaged OpenAnt copies can be relocated.
const SourceCodeBaseEnv = "OPENANT_SOURCE_CODE_BASE"
...
func SourceCodeBaseDir() (string, error) {
	if raw := strings.TrimSpace(os.Getenv(SourceCodeBaseEnv)); raw != "" {
