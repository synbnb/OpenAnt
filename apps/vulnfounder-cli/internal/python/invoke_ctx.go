...
// InvokeCtx runs `python -m vulnfounder <args>` with context-cancellation support.
...
// InvokeCtxCapture runs `python -m vulnfounder <args>` like InvokeCtx but also
...
func invokeCtxInner(ctx context.Context, pythonPath string, args []string, workDir, apiKey string, onLog func(string), captureStdout bool) (string, int, error) {
	// -P keeps the process working directory off sys.path so a hostile vulnfounder/
