...
// VULNFOUNDER_INVOKE_TIMEOUT (or legacy OPENANT_INVOKE_TIMEOUT) without
// recompiling. Completed units are checkpointed,
...
// VulnFounder invoke-timeout environment override. The value is either a Go duration
...
func resolveInvokeTimeout() time.Duration {
	v, source := firstEnvironment("VULNFOUNDER_INVOKE_TIMEOUT", "OPENANT_INVOKE_TIMEOUT")
	if v == "" {
	...
	fmt.Fprintf(os.Stderr,
		"[vulnfounder] ignoring invalid %s=%q; using default %v\n",
		source, v, defaultInvokeTimeout)
	return defaultInvokeTimeout
}
...
// Invoke runs `python -m vulnfounder <args>` and returns the parsed JSON result.
//
// - stderr is streamed to the terminal in real-time (progress messages)
// - stdout is captured and parsed as JSON
// - Working directory is set to the VulnFounder core directory if provided
...
func Invoke(pythonPath string, args []string, workDir string, quiet bool, apiKey string) (*InvokeResult, error) {
	// -P keeps the process working directory off sys.path. `-m vulnfounder` otherwise
	// prepends the CWD, and this engine inherits the user's shell CWD — which in the
	// standard clone-and-scan flow is inside the scanned, untrusted repository. A
	// hostile `vulnfounder/` package there would shadow the real
	...
	cmdArgs := append([]string{"-P", "-m", "vulnfounder"}, args...)
...
// (vulnfounder/cli.py:16 — 0 = clean, 1 = vulnerabilities found, 2 = error) before
