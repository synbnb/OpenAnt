...
func makeProjectRoot(t *testing.T) string {
	t.Helper()
	root := t.TempDir()
	if err := os.MkdirAll(filepath.Join(root, "libs", "vulnfounder-core"), 0o755); err != nil {
		t.Fatalf("mkdir core marker: %v", err)
	}
	if err := os.MkdirAll(filepath.Join(root, "config", "vulnfounder"), 0o700); err != nil {
		t.Fatalf("mkdir config: %v", err)
	}
	for _, marker := range []string{
		filepath.Join(root, "libs", "vulnfounder-core", "pyproject.toml"),
