// Package config handles persistent configuration for the VulnFounder CLI.
//
// Configuration is resolved from an explicit VULNFOUNDER_CONFIG_FILE, the
// project-local config/vulnfounder/config.json, or the legacy VulnFounder
// environment variables and config locations.
...
const (
	// ConfigFileEnv is the primary explicit configuration-file override.
	ConfigFileEnv = "VULNFOUNDER_CONFIG_FILE"
	// LegacyConfigFileEnv remains readable during the VulnFounder migration window.
	LegacyConfigFileEnv = "OPENANT_CONFIG_FILE"

	// ProjectRootEnv identifies the trusted VulnFounder checkout.
	ProjectRootEnv = "VULNFOUNDER_PROJECT_ROOT"
	// LegacyProjectRootEnv keeps existing launch scripts working.
	LegacyProjectRootEnv = "OPENANT_PROJECT_ROOT"

	// DataDirEnv lets packaged and test environments select a data root.
	DataDirEnv = "VULNFOUNDER_DATA_DIR"

	ProjectConfigRelativePath       = "config/vulnfounder/config.json"
	LegacyProjectConfigRelativePath = "config/openant/config.json"
	DefaultLLMConfigName            = "vulnfounder-default"
	LegacyDefaultLLMConfigName      = "openant-default"
)

func IsBuiltinLLMConfigName(name string) bool {
	return name == DefaultLLMConfigName || name == LegacyDefaultLLMConfigName
}
...
// e.g. “vulnfounder set-api-key“) doesn't silently wipe whatever the
// user authored.
type Config struct {
...
// configDir returns the primary user directory for VulnFounder config files.
// Project-local resolution is handled by Path/configCandidates below.
func configDirFor(name string) (string, error) {
...
func configDir() (string, error) {
	return configDirFor("vulnfounder")
}
...
func firstConfiguredEnv(primary, legacy string) (value string, source string) {
	if raw := strings.TrimSpace(os.Getenv(primary)); raw != "" {
		return raw, primary
	}
	if raw := strings.TrimSpace(os.Getenv(legacy)); raw != "" {
		return raw, legacy
	}
	return "", ""
}
...
//
// An explicit file wins. Otherwise a binary inside a VulnFounder checkout uses
...
func Path() (string, error) {
	if raw, _ := firstConfiguredEnv(ConfigFileEnv, LegacyConfigFileEnv); raw != "" {
...
func configCandidates() ([]string, error) {
	...
	if root != "" {
		paths = append(paths,
			filepath.Join(root, filepath.FromSlash(ProjectConfigRelativePath)),
			filepath.Join(root, filepath.FromSlash(LegacyProjectConfigRelativePath)),
		)
	}
	primary, err := userConfigPath()
	if err != nil {
		return nil, err
	}
	legacy, err := legacyUserConfigPath()
	if err != nil {
		return nil, err
	}
...
// ProjectRoot finds the trusted VulnFounder checkout that owns project-local
// files. It intentionally walks only from the selected executable (or an
// explicit project-root environment variable), never from the scanned
// repository's CWD.
func ProjectRoot() (string, error) {
	if raw, source := firstConfiguredEnv(ProjectRootEnv, LegacyProjectRootEnv); raw != "" {
		root, err := absolutePath(raw)
		if err != nil {
			return "", err
		}
		if !isProjectRoot(root) {
			return "", fmt.Errorf("%s=%q is not a VulnFounder project root (missing libs/vulnfounder-core/pyproject.toml or config/models.json)", source, root)
		}
		return root, nil
	}
...
func isProjectRoot(root string) bool {
	models := regularFile(filepath.Join(root, "config", "models.json"))
	primaryCore := regularFile(filepath.Join(root, "libs", "vulnfounder-core", "pyproject.toml"))
	legacyCore := regularFile(filepath.Join(root, "libs", "openant-core", "pyproject.toml"))
	return models && (primaryCore || legacyCore)
}
...
func (c *Config) DefaultLLMName() string {
	...
	return DefaultLLMConfigName
}

// LLMPhaseSummaries returns the phase bindings authored for name in the v2
// config. The built-in VulnFounder default config is defined by Python and has no
...
func (c *Config) LLMPhaseSummaries(name string) []LLMPhaseSummary {
	if c == nil || c.raw == nil || name == "" || IsBuiltinLLMConfigName(name) {
		return nil
	}
...
// DataDir returns the VulnFounder data root. Fresh installs use
// ~/.vulnfounder. When that directory does not yet exist but ~/.openant does,
// the legacy directory remains active so historical scans stay visible.
func DataDir() (string, error) {
	...
	primary := filepath.Join(home, ".vulnfounder")
	legacy := filepath.Join(home, ".openant")
	if regularDir(primary) {
		return primary, nil
	}
	if regularDir(legacy) {
		return legacy, nil
	}
	return primary, nil
}
...
// ProjectsDir returns <data-dir>/projects/.
...
// Name is "org/repo", so the path is <data-dir>/projects/org/repo/.
...
// <data-dir>/projects/org/repo/scans/{shortSHA}/{language}/
