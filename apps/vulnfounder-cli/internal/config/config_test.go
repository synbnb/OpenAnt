...
func withConfigJSON(t *testing.T, body string) {
	t.Helper()
	tmp := t.TempDir()
	subdir := filepath.Join(tmp, "vulnfounder")
...
// ---------------------------------------------------------------------------
// LLM setup helpers — used by ``vulnfounder setup llm``.
// ---------------------------------------------------------------------------
...
func TestDefaultLLMNameFallsBackToBuiltin(t *testing.T) {
	...
	if got := cfg.DefaultLLMName(); got != DefaultLLMConfigName {
		t.Fatalf("DefaultLLMName = %q, want %s", got, DefaultLLMConfigName)
	}
	if got := cfg.LLMPhaseSummaries(DefaultLLMConfigName); got != nil {
		t.Fatalf("built-in phase summaries = %#v, want nil", got)
	}
	if got := cfg.LLMPhaseSummaries(LegacyDefaultLLMConfigName); got != nil {
		t.Fatalf("legacy built-in phase summaries = %#v, want nil", got)
	}
}
...
func TestLLMConfigExistsAndNames(t *testing.T) {
  ...
  "llm_configs": {
	...
	if cfg.LLMConfigExists(DefaultLLMConfigName) || cfg.LLMConfigExists(LegacyDefaultLLMConfigName) {
		t.Error("built-in aliases leaked into LLMConfigExists; they should be invisible to this check")
	}
