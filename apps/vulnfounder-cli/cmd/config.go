...
var configCmd = &cobra.Command{
	...
	Long: `View and update VulnFounder CLI settings.

Configuration is stored in the project-local config/vulnfounder/config.json when
running from a VulnFounder checkout. Existing config/openant/config.json files
and the legacy user directory remain readable during migration.

Examples:
  vulnfounder config set api-key        Set your Anthropic API key (interactive)
  vulnfounder config show               View current configuration
  vulnfounder config unset api-key      Remove your API key
  vulnfounder config path               Print the config file path`,
}
...
Examples:
  vulnfounder config set api-key              Interactive prompt (recommended)
  echo "sk-ant-..." | vulnfounder config set api-key --stdin   Piped input`,
