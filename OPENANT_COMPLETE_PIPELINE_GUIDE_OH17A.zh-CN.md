# OpenAnt 完整扫描主流程学习笔记（截至 OH-17A）

本文按照 [`OPENANT_COMPLETE_PIPELINE_GUIDE.zh-CN.md`](OPENANT_COMPLETE_PIPELINE_GUIDE.zh-CN.md) 的结构，记录当前 OpenHarmony 改造已经实际实现到的阶段。
...
> 一个应用或系统组件从别的进程收到一条请求后，数据经过哪些 C/C++ 函数，最后执行了什么操作？OpenAnt 怎样把这条路径找出来并交给后续安全分析？

OpenHarmony 是一个操作系统。它的源码不是一个单独的程序，而是很多组件组成的大型代码库，例如窗口管理、音频、文件管理、设备驱动和 Ability 运行时。本文只讨论 OpenAnt 如何读取其中一个组件的源码并建立 IPC 语义关系。
...
| C pipeline | OpenAnt 中负责 C/C++ 扫描的流水线，不是 OpenHarmony 的一个系统组件 | 执行扫描、抽取、调用图和 Unit 生成 |
| Tree-sitter | OpenAnt 使用的源码语法解析器，像“读懂 C++ 句子结构的扫描器” | 抽取函数和调用表达式 |
...
| Unit | OpenAnt 交给后续分析的一个分析单元，通常包含一个主函数和有限上下文 | 生成 `dataset.json` |
...
OpenAnt 的工作不是运行这段请求，而是从 IDL、C++ 函数和静态调用图中把这条路径复原出来。
...
`repo_path` 不是某个 C++ 文件，而是一个目录。OpenAnt 会在这个目录下面递归寻找源码、IDL 和构建文件。`platform = openharmony` 相当于告诉扫描器：“这里的目录和入口规则要按 OpenHarmony 组件来理解”。
...
这里的 `code` 可以理解为“菜单编号”，`CMD_ENABLE_SENSOR` 表示菜单中的某一项；`sensorId` 是从外部请求读出的数据。OpenAnt 需要把第 3 步的请求编号和第 4 步的函数对应起来，这正是 `transaction_to_handler` 想表达的关系。
...
如果不沿调用图继续走，OpenAnt 只能看到 `OnRemoteRequestInner`，看不到最后的 `Handle...`。但如果无限制地继续走，又可能把大量普通业务函数全部带进来，所以这里使用固定最大深度。
