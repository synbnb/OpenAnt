...
~~~text
~/.vulnfounder/webui/source-locator/<locator-id>/（旧的 `~/.openant/` 目录仍可作为兼容回退）
~~~
...
如果仓库根目录有 VULNFOUNDER.THREATMODEL.md，会优先加载它（同时兼容旧的
OPENANT.THREATMODEL.md），再与 OpenHarmony
...
~~~text
~/.vulnfounder/webui/<scan-id>/（旧的 `~/.openant/` 目录仍可作为兼容回退）
  meta.json
  logs.txt
...
`VulnFounder/source_code_base/startup_init`，并在 Web 中点击“运行普通扫描”。
扫描器按以下顺序处理：
