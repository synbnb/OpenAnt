...
![VulnFounder 系统运行架构](figures/vulnfounder-system-runtime-architecture.png)

```mermaid
flowchart LR
    subgraph USER["交互层"]
        WEB["Web 浏览器"]
        CLI["VulnFounder CLI<br/>当前命令：vulnfounder"]
    end
    subgraph GO["Go 控制平面"]
...
Go 侧位于 `apps/vulnfounder-cli`，负责用户工作区和进程生命周期，主要职责包括：
...
Python 侧位于 `libs/vulnfounder-core`，负责分析语义：
