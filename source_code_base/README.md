...
这个目录是 OpenAnt 随项目一起使用的本地源码库根目录。每个一级子目录都是一个独立的 Git 仓库，例如：
...
- 一级子目录保留各自的 `.git`、提交历史和 `origin` 远程地址；它们不是 OpenAnt 外层 Git 仓库中的普通源码文件。
...
- 本目录不保存 API 密钥、扫描结果或运行时配置。密钥仍应放在 OpenAnt 的配置目录中，并使用适当的文件权限保护。
