...
| 阶段 2 | 协议恢复 Agent Loop、字段/入口证据、失败反馈 | 已有主要能力 | entry discovery loop、descriptor synthesizer、路由切片、字段证据和 legal probe 校验 | 尚需以 24 项为对象完成协议候选覆盖统计，不能只依赖已有注册描述符 |
| 阶段 3 | HAP/native/CLI/event carrier 和身份阶梯 | 已实现 HAP/native 基础能力 | HAP、Unix native、身份降权字段、载荷 manifest/交付物展示 | CLI/event_bus 通用载体和 24 项载体选择回归仍待完成；HAP 不是所有协议的唯一载体 |
...
这 74 项覆盖的是设备版本比较、协议描述符证据核验、入口/路由循环、载体校验、通用预言机和运行器契约形状。它们证明代码级基础行为，但不等价于当前开发板上 24 个样本全部成功。
...
1. 为 `cli` 和 `event_bus` 增加契约驱动的安全命令数组载体，不把命令拼成未经约束的 shell 字符串。
2. 在 validator 中校验 CLI/event 的命令数组、占位符闭合、身份和事件字段形状。
3. 让 runner 按 `entry.kind` 选择 HAP、native、CLI 或 event carrier，并把发送结果统一写入 `SendResult`。
4. 为合法探针记录“发送命令、设备回执、服务日志/响应和 route_id”。
5. 当描述符失败时保留候选和补证任务，不退回某个样本专用帧作为默认答案。
