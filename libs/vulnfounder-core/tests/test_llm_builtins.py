"""Pin the shape of ``vulnfounder-default`` and the ``report`` CLI dispatch.
...
mode here is "you changed vulnfounder-default — was that intentional?".
...
from utilities.llm import VULNFOUNDER_DEFAULT, OPENANT_DEFAULT, PHASES, PhaseBinding, get_builtin_default


class TestVulnFounderDefault:
    def test_name_is_stable(self):
        assert VULNFOUNDER_DEFAULT.name == "vulnfounder-default"

    def test_legacy_constant_is_an_alias(self):
        """The old Python constant remains a compatibility alias only."""
        assert OPENANT_DEFAULT is VULNFOUNDER_DEFAULT
    ...
    def test_covers_every_phase_explicitly(self):
        ...
        assert set(VULNFOUNDER_DEFAULT.phases) == set(PHASES)
    ...
    def test_every_phase_points_at_anthropic_provider(self):
        ...
        for phase, ref in VULNFOUNDER_DEFAULT.phases.items():
            assert ref.provider == "anthropic", (
                f"vulnfounder-default phase {phase!r} must use provider 'anthropic' "
    ...
    def test_historical_model_assignment(self):
        ...
        # the CHANGELOG.
        assert VULNFOUNDER_DEFAULT.phases["analyze"].model == "claude-opus-4-8"
        assert VULNFOUNDER_DEFAULT.phases["verify"].model == "claude-opus-4-8"
        assert VULNFOUNDER_DEFAULT.phases["llm_reach"].model == "claude-opus-4-8"
        # report restored to Opus (H1) — matches master's report generator.
        assert VULNFOUNDER_DEFAULT.phases["report"].model == "claude-opus-4-8"
        assert VULNFOUNDER_DEFAULT.phases["enhance"].model == "claude-sonnet-4-6"
        assert VULNFOUNDER_DEFAULT.phases["dynamic_test"].model == "claude-sonnet-4-6"
        assert VULNFOUNDER_DEFAULT.phases["app_context"].model == "claude-sonnet-4-6"
    ...
    def test_accessor_returns_same_object(self):
        ...
        assert get_builtin_default() is VULNFOUNDER_DEFAULT
