"""Bug (N4): the JavaScript reachability filter silently blacks out a no-entry-point
dataset.
...
(JavaScript) This drives the REAL `apply_reachability_filter` (real EntryPointDetector + real
...
_TP = _CORE / "parsers" / "javascript" / "test_pipeline.py"
...
def _load_pipeline():
    for p in (str(_TP.parent), str(_CORE)):
        if p not in sys.path:
            sys.path.insert(0, p)
    spec = importlib.util.spec_from_file_location("isolated_javascript_test_pipeline", _TP)
...
_FUNCTIONS = {
    "lib.js:add": {"name": "add", "unit_type": "function", "file_path": "lib.js", "code": "return a + b"},
    "lib.js:scale": {"name": "scale", "unit_type": "function", "file_path": "lib.js",
                    "code": "return add(a, a)"},
}
_UNITS = [
    {"id": "lib.js:add",
     "metadata": {"direct_calls": [], "direct_callers": ["lib.js:scale"]}},
    {"id": "lib.js:scale",
     "metadata": {"direct_calls": ["lib.js:add"], "direct_callers": []}},
]
...
def _make_pipeline():
    ...
    p = mod.PipelineTest(repo_path=str(tmp))
...
def test_empty_seed_keeps_all_units():
    ...
    assert p.reachable_units == {"lib.js:add", "lib.js:scale"}, (
...
def test_written_dataset_retains_every_unit_at_zero_reduction():
    ...
    assert ids == {"lib.js:add", "lib.js:scale"}, "units were dropped (blackout)"
