...
class TestResolveLLMConfig:
    ...
    def test_unknown_name_raises_with_available_list(self):
        ...
        # Both the canonical builtin and user-defined names should be listed.
        assert "vulnfounder-default" in msg
        assert "foo" in msg
...
class TestResolveProvider:
    ...
    def test_anthropic_fallback_when_not_defined(self):
        ...
        # nothing in config.json. vulnfounder-default still resolves
...
class TestLoadConfigFile:
    ...
    def test_project_local_config_precedes_legacy_user_config(self, tmp_path: Path, monkeypatch):
        root = tmp_path / "VulnFounder"
        (root / "libs" / "vulnfounder-core").mkdir(parents=True)
        (root / "config" / "vulnfounder").mkdir(parents=True)
        (root / "libs" / "vulnfounder-core" / "pyproject.toml").write_text("{}\n", encoding="utf-8")
        (root / "config" / "models.json").write_text("{}\n", encoding="utf-8")
        local = root / "config" / "vulnfounder" / "config.json"
    ...
    def test_explicit_missing_config_does_not_fall_back(self, tmp_path: Path, monkeypatch):
        ...
        assert load_config_file().default_llm == "vulnfounder-default"
