...
VULNFOUNDER.THREATMODEL.md (or its legacy OPENANT.THREATMODEL.md alias; a scanned
repo can commit it and it is auto-loaded on every default scan). Each is spliced
onto its own prompt line, so any interpolated value
...
Without a threat model VulnFounder hardcodes a single attacker — "on the internet,
...
def render_attacker_personas(ctx) -> str:
    ...
    for profile in profiles:
        # Every profile field is attacker-authored (from the canonical or legacy
        # threat-model file) and
