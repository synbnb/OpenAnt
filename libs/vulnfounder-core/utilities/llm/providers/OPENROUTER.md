...
VulnFounder's `openrouter` provider type routes pipeline phases through
...
- **Models without a direct adapter.** Any tool-calling model in
  ...
  VulnFounder has no adapter for.
...
There is nothing to install beyond VulnFounder itself — the adapter uses the
same `openai` SDK the OpenAI adapter ships with.

## Configuration

The `vulnfounder setup llm` wizard offers `openrouter` (leave the base URL blank
to use `https://openrouter.ai/api/v1`), or configure the resolved VulnFounder config
file (`config/vulnfounder/config.json` inside a checkout; `VULNFOUNDER_CONFIG_FILE` can
override it)
...
```bash
vulnfounder scan /path/to/repo --llm-config via-openrouter
```
...
(`HTTP-Referer: https://github.com/knostic/VulnFounder`, `X-Title:
VulnFounder`). They only affect OpenRouter's public app-usage rankings —
