...
``config/vulnfounder/config.json`` (with ``config/openant/config.json`` as a
legacy fallback), or the legacy user config directory::

    {
      "$schema_version": 2,
      "default_llm": "vulnfounder-default",
...
``vulnfounder llm-config show vulnfounder-default`` so users can see the
...
# The closed set of phase names. User configs and the built-in default both
...
@dataclass(frozen=True)
class LLMConfig:
    """One entry in ``llm_configs`` (or the built-in VulnFounder default).
    ...
    def __post_init__(self) -> None:
        ...
        if missing or extras:
            ...
            raise ConfigError(
                ...
                f"Run `vulnfounder llm-config show vulnfounder-default` to see the "
...
class ConfigFile:
    ...
    default_llm: str = "vulnfounder-default"
...
def parse_config(raw: dict) -> ConfigFile:
    ...
    default_llm = raw.get("default_llm") or "vulnfounder-default"
...
def _parse_configs(raw: dict) -> dict[str, LLMConfig]:
    ...
    for name, entry in raw.items():
        ...
        if name in {"vulnfounder-default", "openant-default"}:
            raise ConfigError(
                f"config.json: {name!r} is a built-in name and cannot be "
...
def _validate_phase_references(cf: ConfigFile) -> None:
    ...
    config.json) flow through here — the built-in default is
