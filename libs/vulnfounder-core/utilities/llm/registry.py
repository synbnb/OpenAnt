...
1. ``load_config_file()`` resolves ``VULNFOUNDER_CONFIG_FILE`` first, then its
   legacy ``OPENANT_CONFIG_FILE`` alias, the project-local
   ``config/vulnfounder/config.json``, and finally legacy VulnFounder locations.
2. ``resolve_llm_config(cf, name)`` picks the active llm-config by
   name; falls through ``--llm-config`` flag → ``project.json``
   override → file ``default_llm`` → built-in ``vulnfounder-default``.
...
CONFIG_FILE_ENV = "VULNFOUNDER_CONFIG_FILE"
LEGACY_CONFIG_FILE_ENV = "OPENANT_CONFIG_FILE"
PROJECT_ROOT_ENV = "VULNFOUNDER_PROJECT_ROOT"
LEGACY_PROJECT_ROOT_ENV = "OPENANT_PROJECT_ROOT"
PROJECT_CONFIG_RELATIVE_PATH = Path("config/vulnfounder/config.json")
LEGACY_PROJECT_CONFIG_RELATIVE_PATH = Path("config/openant/config.json")


def _first_configured_env(primary: str, legacy: str) -> tuple[str, str]:
...
def _is_project_root(path: Path) -> bool:
    return (
        path.is_dir()
        and (
            (path / "libs" / "vulnfounder-core" / "pyproject.toml").is_file()
            or (path / "libs" / "openant-core" / "pyproject.toml").is_file()
        )
...
def _project_root() -> Optional[Path]:
    ...
    VULNFOUNDER_PROJECT_ROOT (or its legacy alias) when the Python package is
    installed elsewhere.
    """
    explicit, source = _first_configured_env(PROJECT_ROOT_ENV, LEGACY_PROJECT_ROOT_ENV)
    if explicit:
        root = Path(explicit).expanduser().resolve()
        if not _is_project_root(root):
            raise ConfigError(
                f"{source}={str(root)!r} is not a VulnFounder project root "
                "(missing libs/vulnfounder-core/pyproject.toml or config/models.json)"
            )
        return root
...
def _user_config_path(name: str = "vulnfounder") -> Path:
    xdg = os.environ.get("XDG_CONFIG_HOME", "").strip()
    if xdg:
        return Path(xdg).expanduser() / name / "config.json"
    return Path.home() / ".config" / name / "config.json"


def _config_candidates() -> tuple[Path, ...]:
    explicit, _ = _first_configured_env(CONFIG_FILE_ENV, LEGACY_CONFIG_FILE_ENV)
    if explicit:
    ...
    if root is not None:
        ...
        candidates.append(root / LEGACY_PROJECT_CONFIG_RELATIVE_PATH)
    candidates.append(_user_config_path("vulnfounder"))
    candidates.append(_user_config_path("openant"))
...
def default_config_path() -> Path:
    ...
    Mirrors the Go CLI: an explicit ``VULNFOUNDER_CONFIG_FILE`` or legacy
    ``OPENANT_CONFIG_FILE`` wins, followed by the project-local path and then
    the user paths. The project-root check is anchored to this installed
    package or the project-root environment variables;
...
def load_config_file(path: Optional[Path] = None) -> ConfigFile:
    ...
    the caller can still resolve ``vulnfounder-default``.
    """
...
def resolve_llm_config(cf: ConfigFile, name: Optional[str]) -> LLMConfig:
    ...
    2. ``cf.default_llm``.
    3. Built-in ``vulnfounder-default``.
    ...
    if chosen_name in {"vulnfounder-default", "openant-default"}:
        return builtin
    ...
    # silently falling back to the builtin would mask typos.
    available = ["vulnfounder-default"] + sorted(cf.llm_configs)
    raise ConfigError(
...
def resolve_provider(cf: ConfigFile, name: str) -> ProviderConfig:
    ...
    entry in config.json. In that case the built-in config
...
def build_adapter(provider: ProviderConfig) -> LLMAdapter:
    ...
    :class:`LLMAuthError` so the user sees VulnFounder's message
...
def probe_registry_or_raise(registry: PhaseRegistry) -> None:
    ...
    validation: ..." line whether the user ran ``vulnfounder scan`` or
    ``vulnfounder analyze`` standalone.
