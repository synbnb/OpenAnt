...
# The task schema ID is a persisted wire identifier and is intentionally kept
# stable for readers of existing task packages.  New task files use the
# VulnFounder path below; a legacy copy is emitted during the migration window
# so older Claude task instructions continue to resolve.
SKILL_RELATIVE_PATH = Path(".claude/skills/vulnfounder-openharmony-dynamic")
LEGACY_SKILL_RELATIVE_PATH = Path(".claude/skills/openant-openharmony-dynamic")
...
def _project_root() -> Path:
    # .../VulnFounder/libs/vulnfounder-core/utilities/dynamic_tester/claude_code.py
...
def _toolchain_root() -> Path:
    override = first_env("VULNFOUNDER_DYNAMIC_TOOLCHAIN", "OPENANT_DYNAMIC_TOOLCHAIN")
    if override:
...
- 动态测试 Skill：`.claude/skills/vulnfounder-openharmony-dynamic/SKILL.md`
...
def create_claude_code_task(
    ...
    context_dir.mkdir()
    results_dir.mkdir()
    skill_dir = task_dir / SKILL_RELATIVE_PATH
    ...
    # Keep a physical legacy copy rather than a symlink: task packages are
    # routinely moved between hosts and may be archived without preserving
    # links. Both paths point to the same generated instructions.
    legacy_skill = task_dir / LEGACY_SKILL_RELATIVE_PATH
    legacy_skill.mkdir(parents=True)
    shutil.copyfile(skill_dir / "SKILL.md", legacy_skill / "SKILL.md")
