...
# Root of vulnfounder-core (where parsers/ lives)
...
def parse_repository(
    ...
    """Parse a repository into an VulnFounder dataset.
...
def _ensure_js_parser_dependencies() -> None:
    ...
    Mirrors the Go CLI's venv bootstrap (apps/vulnfounder-cli/internal/python/runtime.go):
    ...
    if not (_JS_PARSER_DIR / "package.json").is_file():
        raise RuntimeError(
            ...
            "The vulnfounder-core install may be incomplete."
        )
    ...
    lock_path = _JS_PARSER_DIR / ".vulnfounder-npm-install.lock"
