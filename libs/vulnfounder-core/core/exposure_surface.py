...
# The versioned exposure protocol remains stable during the rename; readers and
# writers intentionally keep the historical identifier so existing device
# asset sessions remain readable. Product-facing labels use VulnFounder.
...
def resolve_hdc_path() -> str:
    ...
    override = first_env("VULNFOUNDER_HDC", "OPENANT_HDC")
    ...
    candidates.append(
        project_root
        / "libs/vulnfounder-core/utilities/dynamic_tester/toolchains/commandline-tools-mac-arm64-6.1.0.860/command-line-tools/sdk/default/openharmony/toolchains/hdc"
    )
    for candidate in candidates:
        ...
        if resolved.is_file() and os.access(resolved, os.X_OK):
            return str(resolved)
    raise ExposureCommandError("未找到可用 HDC，请配置 VULNFOUNDER_HDC（兼容 OPENANT_HDC）或将 hdc 放入 PATH")
