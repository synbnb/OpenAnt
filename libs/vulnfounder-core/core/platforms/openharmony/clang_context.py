...
from utilities.branding import data_dir, first_env
...
def _repository_roots(root: Path) -> list[Path]:
    ...
    configured = first_env("VULNFOUNDER_SOURCE_ROOTS", "OPENANT_SOURCE_ROOTS")
    roots.extend(_path(item) for item in configured.split(os.pathsep) if item.strip())
    # The bundled SDK/tooling checkout is useful for local VulnFounder evaluation,
    # but remains only a fallback; production deployments can supply roots via
    # VULNFOUNDER_SOURCE_ROOTS (or its legacy alias) instead of depending on
    # this layout.
    try:
        ...
        pass
    # Reuse dependency bundles produced by a previous VulnFounder evaluation when
    ...
    cache_root = data_dir() / "evaluation"
...
def _find_candidate_compiler(root: Path) -> tuple[str | None, dict[str, Any]]:
    ...
    for variable, legacy in (("VULNFOUNDER_OHOS_CLANGXX", "OPENANT_OHOS_CLANGXX"), ("OHOS_CLANGXX", None), ("CLANGXX", None)):
        value = first_env(variable, legacy)
        if value:
            candidates.append(_path(value))
    try:
        project_root = Path(__file__).resolve().parents[5]
        toolchains = project_root / "libs" / "vulnfounder-core" / "utilities" / "dynamic_tester" / "toolchains"
...
def prepare_clang_context(
    ...
    *,
    ...
    if output_dir:
        ...
        destination = Path(tempfile.gettempdir()) / "vulnfounder-clang-context" / key
    report: dict[str, Any] = {
