...
``config/languages.json`` is shared. Each record carries ``status`` (VulnFounder's
...
def find_models_config() -> Path | None:
    ...
    """
    override = first_env("VULNFOUNDER_MODELS_CONFIG", "OPENANT_MODELS_CONFIG")
    if override:
        candidate = Path(override)
        if candidate.is_file():
            return candidate
        print(
            f"[models] VULNFOUNDER_MODELS_CONFIG (or legacy OPENANT_MODELS_CONFIG)={override!r} not found; "
...
def require_models() -> list[dict]:
    ...
    models = load_models()
    if not models:
        searched = first_env("VULNFOUNDER_MODELS_CONFIG", "OPENANT_MODELS_CONFIG") or "$VULNFOUNDER_MODELS_CONFIG (unset)"
        raise RuntimeError(
