...
This is the implementation behind ``vulnfounder scan <path>``.
...
def _build_config_id(
    *,
    ...
    return "vulnfounder-build:" + hashlib.sha256(encoded).hexdigest()[:20]
...
def scan_repository(
    ...
    Orchestrates the full VulnFounder pipeline:
    ...
    if generate_context and HAS_APP_CONTEXT:
        ...
        with step_context("app-context", output_dir, inputs={
            ...
            if threat_model_ctx is not None:
                _print_chinese_log(
                    "上下文决策：仓库提供了 VULNFOUNDER.THREATMODEL.md（兼容读取 OPENANT.THREATMODEL.md），"
                    "优先使用该人工安全模型；"
        ...
        threat_model_paths = (
            Path(repo_path) / "VULNFOUNDER.THREATMODEL.md",
            Path(repo_path) / "OPENANT.THREATMODEL.md",
        )
        if any(path.exists() for path in threat_model_paths):
            print(
                "  NOTE: this repository ships a VULNFOUNDER.THREATMODEL.md "
                "(or legacy OPENANT.THREATMODEL.md), which "
                "--no-context discards. The scan will NOT use its attacker "
                "profiles or vulnerability criteria.",
                file=sys.stderr,
            )
            _print_chinese_log(
                "重要提示：仓库虽然提供了 VULNFOUNDER.THREATMODEL.md（或兼容的 OPENANT.THREATMODEL.md），"
                "但本次 --no-context 已明确丢弃它；"
                "结果不能按该模型解释。"
            )
...
def _print_banner(
    ...
    print("VULNFOUNDER SCAN", file=sys.stderr)
    print("=" * 60, file=sys.stderr)
...
def _print_summary(result: ScanResult) -> None:
    ...
    if result.usage.total_input_tokens == 0 and result.metrics.errors > 0:
        ...
        print("  *** Check your API key: vulnfounder set-api-key <key>       ***", file=sys.stderr)
    print("=" * 60, file=sys.stderr)
