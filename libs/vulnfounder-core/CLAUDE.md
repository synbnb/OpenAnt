...
**NEVER run `make install` in `apps/vulnfounder-cli/`** — it overwrites the symlink with a copy.

The canonical launcher is `vulnfounder`: `/usr/local/bin/vulnfounder` → `apps/vulnfounder-cli/bin/vulnfounder`.
`openant` remains an explicitly supported compatibility alias and may point to
`apps/vulnfounder-cli/bin/openant` when `make build-legacy` has been run.

To rebuild the Go CLI:
```bash
cd apps/vulnfounder-cli && go build -o bin/vulnfounder .
```
...
This is VulnFounder, a two-stage SAST tool using Claude for vulnerability analysis. Supports Python, JavaScript/TypeScript, Go, C/C++, Ruby, PHP, Zig, Swift, and Rust codebases with 4-level cost optimization.
...
VulnFounder uses application context to understand what type of application it's analyzing, which dramatically reduces false positives by understanding what behaviors are intentional vs vulnerable.
...
**Manual override:** Create `VULNFOUNDER.md` or `VULNFOUNDER.json` in repo root. See
`context/VULNFOUNDER_TEMPLATE.md` for the format. Existing `OPENANT.md` and
`OPENANT.json` files remain readable as legacy aliases.

**Unsupported types:** If a repository doesn't match supported types, VulnFounder exits with error code 2 and instructions for creating a manual override.
