# OpenAnt 面向 OpenHarmony 的完整目标态扫描流程指南

> 文档性质：本文描述 OpenAnt 面向 OpenHarmony 源码扫描和开发板验证的完整目标架构，用现在时连贯说明最终产品流程，适合方案汇报、架构讲解和后续实施对照。本文不是当前代码版本的功能验收证明；具体功能是否已经可运行，应以对应代码、阶段测试记录和真实设备测试结果为准。
...
- [`OPENANT_COMPLETE_PIPELINE_GUIDE_OH17A.zh-CN.md`](OPENANT_COMPLETE_PIPELINE_GUIDE_OH17A.zh-CN.md)
- [`OPENANT_COMPLETE_PIPELINE_GUIDE.zh-CN.md`](OPENANT_COMPLETE_PIPELINE_GUIDE.zh-CN.md)
...
项目元数据保存在项目目录中，扫描产物保存在每次独立 scan 目录中。源码库、OpenAnt 代码和扫描结果物理上可随项目一起打包，但 API 密钥、签名私钥和设备隐私数据不进入仓库。
...
它不会取代 C/C++ 文件解析，也不会单独决定入口。它只是告诉 OpenAnt “这份源码属于什么组件、会被编译成什么”。
...
OpenAnt 遍历受支持源码文件，按文件数量和占比选择语言。`auto` 模式可以并行解析多个达到阈值的语言，避免只扫描主语言而漏掉 NAPI、ArkTS 或辅助脚本。
...
如果没有识别到可信入口，OpenAnt 不把 dataset 清空，而是：
...
```bash
./apps/openant-cli/bin/openant scan \
...
```bash
./apps/openant-cli/bin/openant ohos-dynamic plan \
...
```bash
./apps/openant-cli/bin/openant ohos-dynamic preflight \
  --device <serial> \
  --output <scan-dir>/openharmony_dynamic/<run-id>
```
...
```bash
./apps/openant-cli/bin/openant ohos-dynamic run \
  --device <serial> \
...
```bash
./apps/openant-cli/bin/openant ohos-dynamic analyze \
  --run <scan-dir>/openharmony_dynamic/<run-id>

./apps/openant-cli/bin/openant report \
  <scan-dir>/pipeline_output.json \
  --format html \
...
```text
1. OpenAnt 接收 OpenHarmony 组件仓库和平台配置。
